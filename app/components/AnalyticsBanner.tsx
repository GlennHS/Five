...
export default function AnalyticsBanner() {
  ...
  const { acceptedAnalytics, loadingConsent, updateConsent } = useConsent()
    ...
    <div className="w-full flex items-center justify-center text-center fixed top-0 left-0 backdrop-blur-lg z-50">
      <div className="flex items-center justify-around gap-2 max-w-3xl w-full bg-bond/20 text-black border-b-2 border-bond shadow-2xl px-2 py-4 z-50">
        ...
        <div className="flex flex-col items-center justify-center gap-4 text-xs">
          <button onClick={() => updateConsent(true)} className="border border-black rounded-xl bg-bond text-white font-bold active:bg-gray-300 px-4 py-1">Sure!</button>
          <button onClick={() => updateConsent(false)} className="text-black text-2xs italic underline active:bg-gray-300 py-1">No thanks</button>
        </div>
      </div>
    </div>
  )
